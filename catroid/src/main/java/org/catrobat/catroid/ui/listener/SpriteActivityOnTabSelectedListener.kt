...
class SpriteActivityOnTabSelectedListener(val loadFragment:kotlin.reflect.KFunction1<Int,
    Unit>) :
...
fun SpriteActivity.loadFragment(fragmentPosition: Int) {
    ...
    when (fragmentPosition) {
        SpriteActivity.FRAGMENT_SCRIPTS -> showScripts(fragmentTransaction)
        SpriteActivity.FRAGMENT_LOOKS -> fragmentTransaction.replace(
            R.id.fragment_container,
            LookListFragment(),
            LookListFragment.TAG
        )
        SpriteActivity.FRAGMENT_SOUNDS -> fragmentTransaction.replace(
            R.id.fragment_container,
            SoundListFragment(),
...
fun SpriteActivity.showScripts(fragmentTransaction:FragmentTransaction) {
